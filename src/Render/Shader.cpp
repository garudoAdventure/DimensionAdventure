...
Shader::Shader(ID3D11Device* device, ID3D11DeviceContext* deviceContext) {
  ...
  {
    std::ifstream ifs_ps("colorGradingPS.cso", std::ios::binary);
    ifs_ps.seekg(0, std::ios::end);
    std::streamsize filesize = ifs_ps.tellg();
    ifs_ps.seekg(0, std::ios::beg);
    unsigned char* psbinary_pointer = new unsigned char[filesize];
    ...
    _device->CreatePixelShader(psbinary_pointer, filesize, nullptr, &_colorGradingShader);
    delete[] psbinary_pointer;
  }
...
void Shader::setPS(PS ps) {
  switch (ps) {
    ...
    case PS::SPIRAL:
      ...
      break;
    case PS::COLOR_GRADING:
      _deviceContext->PSSetShader(_colorGradingShader, nullptr, 0);
      break;
  }
}
