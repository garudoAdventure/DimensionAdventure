...
Field02::Field02(IGameEventHandler* gameEvent) : Field("./assets/map/map2.csv") {
	std::array<Model*, LAYER_NUM> hintWall = {
	...
	for (int i = 0; i < LAYER_NUM; i++) {
		...
		_layer.at(i)->addGameObj(new HintBlock(
			MathTool::getCoordPos({ 14.0f, 3.0f, 5.0f }),
			{ 1.0f, 1.0f, 1.0f }, Color::layerColor.at(i),
			hintWall.at(i)
		));
		_layer.at(i)->addGameObj(new PlayerFallPoint(
			MathTool::getCoordPos({ 29.0f, -15.0f, 5.0f }),
			Float3{ 58.0f, 1.0f, 10.0f } * 3,
			gameEvent
		));
		_layer.at(i)->addGameObj(new SavePoint(
	...
	_layer.at(LayerType::WHITE)->addGameObj(new DimensionRing(
		MathTool::getCoordPos({ 12.0f, 2.5f, 5.0f }),
		gameEvent
	));
	_layer.at(LayerType::WHITE)->addGameObj(new RemoteControl(
		MathTool::getCoordPos({ 39.0f, 4.5f, 5.0f }),
		gameEvent
	));
	_layer.at(LayerType::WHITE)->addGameObj(new Epigraph(gameEvent,
