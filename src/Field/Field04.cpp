#include "Field.h"
#include "./Object/Door.h"
#include "./Object/Block.h"
#include "./Object/Epigraph.h"
#include "./Object/PlayerFallPoint.h"
#include "./Object/SavePoint.h"
#include "./Item/Crystal.h"
#include "./Common/Color.h"

Field04::Field04(IGameEventHandler* gameEvent) : Field("./assets/map/map4.csv") , _gameEvent(gameEvent) {
	Model* floorModel = MODEL.loadModel("./assets/model/block.fbx");
	
	for (int i = 0; i < LAYER_NUM; i++) {
		_layer.at(i)->addGameObj(new OpenedDoor(
			MathTool::getCoordPos({ 2.0f, 1.5f, 10.0f }), 3, gameEvent,
			MathTool::getCoordPos({ 3.0f, 1.1f, 5.0f })
		));
		_layer.at(i)->addGameObj(new PlayerFallPoint(
			MathTool::getCoordPos({ 29.0f, -15.0f, 5.0f }),
			Float3{ 58.0f, 1.0f, 10.0f} * 3,
			gameEvent
		));
		_layer.at(i)->addGameObj(new SavePoint(
			MathTool::getCoordPos({ 7.0f, 1.5f, 5.0f }),
			Float3{ 1.0f, 15.0f, 10.0f } * 3,
			gameEvent
		));
		_layer.at(i)->addGameObj(new SavePoint(
			MathTool::getCoordPos({ 34.0f, 1.5f, 5.0f }),
			Float3{ 1.0f, 15.0f, 10.0f } * 3,
			gameEvent
		));
		_layer.at(i)->addGameObj(new MovingFloor(
			MathTool::getCoordPos({ 15.0f, 0.0f, 5.0f }),
			MathTool::getCoordPos({ 10.0f, 0.0f, 5.0f }),
			{ 3.0f, 1.0f, 3.0f },
			Color::layerColor.at(i), floorModel
		));
		_layer.at(i)->addGameObj(new MovingFloor(
			MathTool::getCoordPos({ 15.0f, 0.0f, 8.0f }),
			MathTool::getCoordPos({ 20.0f, 0.0f, 8.0f }),
			{ 3.0f, 1.0f, 3.0f },
			Color::layerColor.at(i), floorModel
		));
		_layer.at(i)->addGameObj(new MovingFloor(
			MathTool::getCoordPos({ 28.0f, 0.0f, 1.0f }),
			MathTool::getCoordPos({ 23.0f, 0.0f, 1.0f }),
			{ 3.0f, 1.0f, 3.0f },
			Color::layerColor.at(i), floorModel
		));
		_layer.at(i)->addGameObj(new MovingFloor(
			MathTool::getCoordPos({ 31.0f, 0.0f, 5.0f }),
			MathTool::getCoordPos({ 31.0f, 5.0f, 5.0f }),
			{ 3.0f, 1.0f, 3.0f },
			Color::layerColor.at(i), floorModel
		));
	}

	_layer.at(LayerType::WHITE)->addGameObj(new RedCrystal(
		MathTool::getCoordPos({ 39.0f, 3.5f, 5.0f }),
		gameEvent
	));
	_layer.at(LayerType::WHITE)->addGameObj(new Epigraph(gameEvent,
		MathTool::getCoordPos({ 7.0f, 2.0f, 10.0f }),
		Color::lightRed,
		L"「お宝は奥にある」と記されている。"
	));

	Field::load();
}